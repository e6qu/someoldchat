...
func (s *Store) CompleteExternalUpload(_ context.Context, id domain.ExternalUploadID, file domain.File, event events.Event) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	value, exists := s.externalUploads[id]
	if !exists {

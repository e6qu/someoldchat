...
// workflowStepUserList splits the builder's comma-separated member field. The
// builder takes identifiers rather than a picker because a workflow may add
// people who are not in the channel it edits, so the list cannot be drawn from
// the conversation's own membership.
func workflowStepUserList(raw string) []string {
	values := strings.Split(raw, ",")
	people := make([]string, 0, len(values))
...
func decodeWorkflowSteps(raw string) []workflowDecodedStep {
	...
	for _, step := range steps {
		decoded := workflowDecodedStep{Callback: step.FunctionID, Type: step.Type}
		if step.Form != nil {
			encoded, err := json.Marshal(map[string]any{
				"title": step.Form.Title, "description": step.Form.Description, "inputs": step.Form.Inputs,
			})
			if err == nil {
				decoded.Form = string(encoded)
			}
		}
		if step.Button != nil {
			decoded.ButtonLabel = step.Button.Label
		}
		if step.Message != nil {
			decoded.MessageConversation = step.Message.Conversation
			decoded.MessageText = step.Message.Text
		}
		// add_people and create_canvas reuse the conversation and text controls
		// so a step keeps one row in the builder whichever kind it is.
		if step.AddPeople != nil {
			decoded.MessageConversation = step.AddPeople.Conversation
			decoded.People = strings.Join(step.AddPeople.Users, ", ")
		}
		if step.Canvas != nil {
			decoded.MessageConversation = step.Canvas.Conversation
			decoded.MessageText = step.Canvas.Title
		}
		// The builder works in minutes because that is the unit Slack's own
		// wait step offers; the definition keeps seconds so a step is not
		// rounded by the surface that edits it.
		if step.Delay != nil && step.Delay.Seconds > 0 {
			decoded.DelayMinutes = strconv.Itoa(step.Delay.Seconds / 60)
		}
		// Shown in the zone the builder submits with, so an author reads back
		// the moment they chose rather than its UTC equivalent.
		if step.WaitUntil != nil && step.WaitUntil.UnixSeconds > 0 {
			decoded.WaitUntil = time.Unix(step.WaitUntil.UnixSeconds, 0).UTC().Format("2006-01-02T15:04")
		}
...
type workflowData struct {
	...
	IsOwner         bool
	CanCopy         bool
	Permissions     []workflowPermissionView
}

// workflowPermissionView is one of the workflow's find/use/copy scopes as the
// permissions panel renders it.
...
type workflowInteractionView struct {
	StepID string
	Kind   string
	Title  string
	Label  string
...
.step-form,.step-button{margin-top:5px;font-family:monospace;font-size:11px;padding:6px!important;border:1px solid var(--field-line);border-radius:6px;background:var(--field);color:var(--text)}
.step-reorder{display:flex;flex-direction:column;gap:2px}.step-move{border:1px solid var(--field-line);border-radius:4px;background:var(--panel-strong);color:var(--text);font-weight:800;padding:1px 6px;font-size:11px;cursor:pointer}.step-move:disabled{opacity:0.4;cursor:default}
.wf-icon{display:grid;place-items:center;width:34px;height:34px;border-radius:8px;background:var(--panel-strong);font-size:18px}
.export-links{margin:0 0 10px;font-size:13px}.export-links a{color:var(--action);font-weight:700;text-decoration:none}
...
{{if .Owned}}<section class="panel" aria-labelledby="builder-heading"><h3 id="builder-heading">Build workflow</h3><p>Steps run from top to bottom. Publishing makes the current version available to its enabled triggers; unpublished workflows can retain draft changes.</p><form class="fields" method="post" action="/app/workflows/{{.ID}}/update"><input type="hidden" name="_csrf" value="{{.CSRFToken}}"><input type="hidden" name="version" value="{{.Version}}"><input type="hidden" name="step_count" value="{{.StepCount}}"><label>Name<input name="title" maxlength="255" value="{{.Title}}" required></label><label>Workflow reference<input name="callback_id" maxlength="255" value="{{.CallbackID}}"></label><label class="wide">Description<textarea name="description" maxlength="2000">{{.Description}}</textarea></label><label>Icon (emoji or short text)<input name="icon" maxlength="64" value="{{.Icon}}" placeholder="🚀"></label><label class="wide">Input metadata (JSON object; syntax validation only)<textarea name="input_schema" spellcheck="false">{{.InputSchema}}</textarea></label><fieldset class="wide"><legend>Steps</legend><div class="step-list">{{range .StepSlots}}{{$slot := .}}<div class="step"><b aria-hidden="true">{{.Number}}</b><span class="step-reorder" data-step-index="{{.Number}}"><button type="button" class="step-move" data-move="up" data-step="{{.Number}}" aria-label="Move step {{.Number}} up"{{if eq .Number 1}} disabled{{end}}>↑</button><button type="button" class="step-move" data-move="down" data-step="{{.Number}}" aria-label="Move step {{.Number}} down"{{if eq .Number $.StepCount}} disabled{{end}}>↓</button></span><label><span><span class="visually-hidden">Step {{.Number}}{{if .Change}} · {{.Change}}{{end}}</span><select name="step_type_{{.Number}}" aria-label="Step {{.Number}} type"><option value="function"{{if or (eq .Type "") (eq .Type "function")}} selected{{end}}>Function</option><option value="form"{{if eq .Type "form"}} selected{{end}}>Form</option><option value="button"{{if eq .Type "button"}} selected{{end}}>Button</option><option value="message"{{if eq .Type "message"}} selected{{end}}>Send a message</option><option value="add_people"{{if eq .Type "add_people"}} selected{{end}}>Add people to a channel</option><option value="create_canvas"{{if eq .Type "create_canvas"}} selected{{end}}>Create a canvas</option><option value="delay"{{if eq .Type "delay"}} selected{{end}}>Wait for a time</option><option value="wait_until"{{if eq .Type "wait_until"}} selected{{end}}>Wait until a date</option></select><select name="step_{{.Number}}"><option value="">{{if eq .Number 1}}Choose a function{{else}}No step{{end}}</option>{{range $.Functions}}<option value="{{.CallbackID}}"{{if eq .CallbackID $slot.Selected}} selected{{end}}>{{.Title}} · {{.CallbackID}}</option>{{end}}</select><input class="step-mapping" name="mapping_{{.Number}}" value="{{.Mapping}}" maxlength="2000" spellcheck="false" placeholder="Input mapping: {&quot;item&quot;:&quot;inputs.item&quot;,&quot;prev&quot;:&quot;steps.id.outputs.x&quot;}" aria-label="Step {{.Number}} input mapping"><input class="step-form" name="form_{{.Number}}" value="{{.Form}}" maxlength="2000" spellcheck="false" placeholder="Form definition: {&quot;title&quot;:&quot;Intake&quot;,&quot;inputs&quot;:{&quot;name&quot;:&quot;Name&quot;}}" aria-label="Step {{.Number}} form definition"><input class="step-button" name="button_label_{{.Number}}" value="{{.ButtonLabel}}" maxlength="255" placeholder="Button label" aria-label="Step {{.Number}} button label"><select class="step-message" name="message_conversation_{{.Number}}" aria-label="Step {{.Number}} message conversation"><option value="">Send a message to…</option>{{range $.Channels}}<option value="{{.ID}}"{{if eq .ID $slot.MessageConversation}} selected{{end}}>#{{.Name}}</option>{{end}}</select><input class="step-message" name="message_text_{{.Number}}" value="{{.MessageText}}" maxlength="4000" placeholder="Message text; {{"{{"}}inputs.name{{"}}"}} quotes a run input" aria-label="Step {{.Number}} message text or canvas title"><input class="step-message" name="step_people_{{.Number}}" value="{{.People}}" maxlength="1000" placeholder="Member IDs to add, comma separated" aria-label="Step {{.Number}} people to add"><input class="step-message" name="step_delay_{{.Number}}" value="{{.DelayMinutes}}" type="number" min="1" max="43200" placeholder="Wait, in minutes" aria-label="Step {{.Number}} wait in minutes"><input class="step-message" name="step_until_{{.Number}}" value="{{.WaitUntil}}" type="datetime-local" step="60" aria-label="Step {{.Number}} wait until">{{if .Change}}<span class="step-change" data-step-change="{{.Number}}" aria-label="Step {{.Number}} {{.Change}}">{{.Change}}</span>{{end}}</span><span class="step-condition"><span class="visually-hidden">Step {{.Number}} condition</span><input name="condition_source_{{.Number}}" value="{{.ConditionSource}}" maxlength="255" placeholder="Only run if: inputs.flag or steps.id.outputs.name" aria-label="Step {{.Number}} condition source"><select name="condition_operator_{{.Number}}" aria-label="Step {{.Number}} condition operator"><option value="">always runs</option><option value="equals"{{if eq .ConditionOperator "equals"}} selected{{end}}>equals</option><option value="not_equals"{{if eq .ConditionOperator "not_equals"}} selected{{end}}>does not equal</option><option value="contains"{{if eq .ConditionOperator "contains"}} selected{{end}}>contains</option><option value="greater_than"{{if eq .ConditionOperator "greater_than"}} selected{{end}}>is greater than</option><option value="less_than"{{if eq .ConditionOperator "less_than"}} selected{{end}}>is less than</option></select><input name="condition_value_{{.Number}}" value="{{.ConditionValue}}" maxlength="255" placeholder="value" aria-label="Step {{.Number}} condition value"></span></label></div>{{end}}</div>{{if .RemovedSteps}}<div class="removed-steps"><strong>Removed from the published version</strong>{{range .RemovedSteps}}<span class="removed-step" data-removed-step="{{.Position}}">{{.Title}} · {{.FunctionID}}</span>{{end}}</div>{{end}}</fieldset><div class="actions">{{if .PublishedStatus}}{{if .StagedEdits}}<button class="secondary" name="action" value="discard" type="submit">Discard changes</button>{{end}}<button name="action" value="save" type="submit">Save staged changes</button><button name="action" value="publish" type="submit">Publish changes</button><button class="secondary" name="action" value="unpublish" type="submit">Unpublish</button>{{else}}<button name="action" value="save" type="submit">Save draft</button><button name="action" value="publish" type="submit">Publish</button>{{end}}</div></form><div class="actions"><form method="post" action="/app/workflows/{{.ID}}/copy"><input type="hidden" name="_csrf" value="{{.CSRFToken}}"><button class="secondary" type="submit">Copy workflow</button></form><form method="post" action="/app/workflows/{{.ID}}/delete"><input type="hidden" name="_csrf" value="{{.CSRFToken}}"><input type="hidden" name="version" value="{{.Version}}"><button class="secondary" type="submit">Delete workflow</button></form></div></section>
 ...
 <section class="panel" aria-labelledby="permissions-heading"><h3 id="permissions-heading">Workflow permissions</h3><p>Control who can find this workflow in the directory, use its link and shortcut triggers, and copy it. Collaborators are the owner and managers; named member IDs apply only to "Named members".</p>{{range .Permissions}}<form class="fields" method="post" action="/app/workflows/{{$.ID}}/permissions"><input type="hidden" name="_csrf" value="{{$.CSRFToken}}"><input type="hidden" name="scope" value="{{.Scope}}"><label>{{.Title}}<select name="permission_type"><option value="everyone"{{if eq .PermissionType "everyone"}} selected{{end}}>Everyone in the workspace</option><option value="app_collaborators"{{if eq .PermissionType "app_collaborators"}} selected{{end}}>Collaborators only</option><option value="named_entities"{{if eq .PermissionType "named_entities"}} selected{{end}}>Named members</option></select></label><label>Named member IDs<input name="user_ids" value="{{.UserIDs}}" placeholder="U123, U456"></label><div class="actions"><button type="submit">Save</button></div></form>{{end}}</section>
 ...
 {{if and .CanCopy (not .Owned)}}<section class="panel" aria-labelledby="copy-heading"><h3 id="copy-heading">Copy workflow</h3><p>Create your own draft copy of this workflow's published version.</p><form method="post" action="/app/workflows/{{.ID}}/copy"><input type="hidden" name="_csrf" value="{{.CSRFToken}}"><button class="secondary" type="submit">Copy workflow</button></form></section>{{end}}
...
func (h Handler) createWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	value, err := h.Messages.CreateWorkflow(r.Context(), principal.WorkspaceID, principal.UserID, domain.WorkflowDefinition{
		...
		Description: strings.TrimSpace(fields["description"]), Icon: strings.TrimSpace(fields["icon"]),
		InputSchema: `{}`, Steps: string(steps),
	})
	if err != nil {
...
func workflowSlots(steps []workflowDecodedStep) []workflowStepSlot {
	...
	for index := range result {
		...
		if index < len(steps) {
			...
			result[index].Type = steps[index].Type
			result[index].Form = steps[index].Form
			result[index].ButtonLabel = steps[index].ButtonLabel
			result[index].MessageConversation = steps[index].MessageConversation
			result[index].MessageText = steps[index].MessageText
			result[index].People = steps[index].People
			result[index].DelayMinutes = steps[index].DelayMinutes
			result[index].WaitUntil = steps[index].WaitUntil
...
func (h Handler) workflow(w http.ResponseWriter, r *http.Request) {
	...
	// The workflow-level scopes: a manager holds them all; anyone else reading
	// this page could already find the workflow, and their use/copy grants
	// decide the run buttons and the copy control below.
	canCopy, canUse := canManage, true
	if !canManage && value.Status == domain.WorkflowPublished {
		for scope, allowed := range map[string]*bool{"use": &canUse, "copy": &canCopy} {
			permission, err := h.Messages.GetWorkflowPermission(r.Context(), principal.WorkspaceID, principal.UserID, id, scope)
			if err != nil {
				h.writeStoreError(w, err, "Workflow permissions are temporarily unavailable.")
				return
			}
			*allowed = workflowScopeAllows(permission, principal)
		}
	}
	permissionViews := make([]workflowPermissionView, 0, 3)
	if canManage {
		for _, scope := range []struct{ scope, title string }{
			{"find", "Who can find this workflow"},
			{"use", "Who can use this workflow"},
			{"copy", "Who can copy this workflow"},
		} {
			permission, err := h.Messages.GetWorkflowPermission(r.Context(), principal.WorkspaceID, principal.UserID, id, scope.scope)
			if err != nil {
				h.writeStoreError(w, err, "Workflow permissions are temporarily unavailable.")
				return
			}
			permissionViews = append(permissionViews, workflowPermissionView{
				Scope: scope.scope, Title: scope.title,
				PermissionType: permission.PermissionType, UserIDs: joinUserIDs(permission.UserIDs),
			})
		}
	}
	...
	for _, trigger := range triggers {
		...
		if trigger.Enabled && value.Status == domain.WorkflowPublished &&
			...
			// Both gates the service enforces at run time: the trigger's own
			// grant and the workflow's use scope.
	...
	h.writeHTML(w, workflowTemplate, workflowData{
		...
		CanCopy: canCopy, Permissions: permissionViews,
	}, http.StatusOK, "workflow rendering unavailable")
}

// workflowScopeAllows evaluates a workflow find/use/copy permission for a
// viewer who is NOT a manager — managers bypass the scopes entirely, and
// "app_collaborators" means exactly them. It mirrors the service's
// enforcement so the page never shows a control the service would refuse.
func workflowScopeAllows(permission domain.AutomationPermission, principal auth.Principal) bool {
...
func encodeWorkflowSteps(fields map[string]string, options []workflowFunctionOption, appID domain.AppID) (string, error) {
	...
	for index := 1; index <= count; index++ {
		...
		case domain.WorkflowStepCanvas:
			...
			canvas := map[string]any{"title": title}
		...
		case domain.WorkflowStepWaitUntil:
			...
			// the author chose.
			zone := strings.TrimSpace(fields["schedule_timezone"])
			if zone == "" {
				zone = "UTC"
			}
			...
			at, err := time.ParseInLocation("2006-01-02T15:04", strings.TrimSpace(fields[fmt.Sprintf("step_until_%d", index)]), location)
			if err != nil {
...
func decodeWorkflowFormDefinition(raw string) (map[string]any, error) {
	if raw == "" {
		return nil, errors.New("a form step needs a title")
	}
	var form struct {
		Title       string            `json:"title"`
		Description string            `json:"description"`
	...
	if json.Unmarshal([]byte(raw), &form) != nil {
		return nil, errors.New("form definition must be a JSON object")
	}
	if strings.TrimSpace(form.Title) == "" {
		return nil, errors.New("a form step needs a title")
	}
	definition := map[string]any{"title": strings.TrimSpace(form.Title)}
	if strings.TrimSpace(form.Description) != "" {
		definition["description"] = strings.TrimSpace(form.Description)
	}
	if len(form.Inputs) > 0 {
		inputs := make(map[string]string, len(form.Inputs))
		for name, label := range form.Inputs {
			if strings.TrimSpace(name) == "" || strings.TrimSpace(label) == "" {
				return nil, errors.New("form fields need a name and a label")
			}
			inputs[strings.TrimSpace(name)] = strings.TrimSpace(label)
		}
...
func (h Handler) updateWorkflow(w http.ResponseWriter, r *http.Request) {
	...
	current.Icon = strings.TrimSpace(fields["icon"])
...
// setWorkflowPermission records one of the workflow's find/use/copy scopes
// from the permissions panel. The service decides who may set it (managers)
// and validates the named members.
func (h Handler) setWorkflowPermission(w http.ResponseWriter, r *http.Request) {
	principal, _, ok := h.workflowPrincipal(w, r)
	if !ok {
		return
	}
	...
	if _, err := h.Messages.SetWorkflowPermission(r.Context(), principal.WorkspaceID, principal.UserID, id, strings.TrimSpace(fields["scope"]), permission); err != nil {
		h.writeWorkflowMutationError(w, r, "The permissions were not updated", err)
		return
	}
	h.redirectMutation(w, r, "/app/workflows/"+url.PathEscape(string(id))+"?notice="+url.QueryEscape("Permissions updated"))
}
...
func (h Handler) workflowRun(w http.ResponseWriter, r *http.Request) {
	...
	interaction, err := h.Messages.WorkflowRunInteraction(r.Context(), principal.WorkspaceID, principal.UserID, run.ID)
	if err != nil {
		h.writeStoreError(w, err, "That workflow run is not available.")
		return
	}
	...
	h.writeHTML(w, workflowRunTemplate, workflowRunData{
		...
		CSRFToken: csrf, Notice: strings.TrimSpace(r.URL.Query().Get("notice")),
		Interaction: view, HasInteraction: view.Kind != "",
	}, http.StatusOK, "workflow run rendering unavailable")
}

func (h Handler) submitWorkflowForm(w http.ResponseWriter, r *http.Request) {
	principal, _, ok := h.workflowPrincipal(w, r)
	if !ok {
		return
	}
	fields, ok := h.decodeMutation(w, r, "Reload the run and try again.")
	if !ok {
		return
	}
	runID := domain.WorkflowRunID(strings.TrimSpace(r.PathValue("runID")))
	stepID := domain.WorkflowStepID(strings.TrimSpace(fields["step_id"]))
	inputs := map[string]string{}
	for name, value := range fields {
		if strings.HasPrefix(name, "field_") {
			inputs[strings.TrimPrefix(name, "field_")] = value
		}
	}
	encoded, err := json.Marshal(inputs)
	if err != nil {
		h.writeMutationError(w, r, http.StatusBadRequest, "The form was not submitted", "Check the form fields and try again.")
		return
	}
	if err := h.Messages.SubmitWorkflowForm(r.Context(), principal.WorkspaceID, principal.UserID, runID, stepID, string(encoded)); err != nil {
		h.writeWorkflowMutationError(w, r, "The form was not submitted", err)
		return
	}
	h.redirectMutation(w, r, "/app/workflows/runs/"+url.PathEscape(string(runID))+"?notice="+url.QueryEscape("Form submitted"))
}

func (h Handler) completeWorkflowButton(w http.ResponseWriter, r *http.Request) {
	principal, _, ok := h.workflowPrincipal(w, r)
	if !ok {
		return
	}
	...
	runID := domain.WorkflowRunID(strings.TrimSpace(r.PathValue("runID")))
	stepID := domain.WorkflowStepID(strings.TrimSpace(fields["step_id"]))
	if err := h.Messages.CompleteWorkflowButton(r.Context(), principal.WorkspaceID, principal.UserID, runID, stepID); err != nil {
		h.writeWorkflowMutationError(w, r, "The button was not confirmed", err)
		return
	}
	h.redirectMutation(w, r, "/app/workflows/runs/"+url.PathEscape(string(runID))+"?notice="+url.QueryEscape("Confirmed"))
}
...
func (h Handler) exportWorkflowRuns(w http.ResponseWriter, r *http.Request) {
	principal, _, ok := h.workflowPrincipal(w, r)
	if !ok {
		return
	}
	id := domain.WorkflowID(strings.TrimSpace(r.PathValue("workflowID")))
	runs, err := h.Messages.WorkflowRunExport(r.Context(), principal.WorkspaceID, principal.UserID, id)
	if err != nil {
		h.writeStoreError(w, err, "That workflow is not available.")
		return
	}
	...
	titles := make(map[domain.WorkflowTriggerID]string, len(triggers))
	for _, trigger := range triggers {
		titles[trigger.ID] = trigger.Title
	}
	rows := make([][]string, 0, len(runs))
	for _, run := range runs {
		completed := ""
		if !run.CompletedAt.IsZero() {
			completed = run.CompletedAt.UTC().Format(time.RFC3339)
		}
		rows = append(rows, []string{
			string(run.ID), titles[run.TriggerID], string(run.Status),
			run.CreatedAt.UTC().Format(time.RFC3339), completed, run.Error,
		})
	}
	writeCSV(w, "workflow-runs.csv",
		[]string{"run_id", "trigger", "status", "started_at", "completed_at", "error"}, rows)
}

func (h Handler) exportWorkflowFormResponses(w http.ResponseWriter, r *http.Request) {
	principal, _, ok := h.workflowPrincipal(w, r)
	if !ok {
		return
	}
	id := domain.WorkflowID(strings.TrimSpace(r.PathValue("workflowID")))
	responses, err := h.Messages.WorkflowFormResponseExport(r.Context(), principal.WorkspaceID, principal.UserID, id)
	if err != nil {
		h.writeStoreError(w, err, "That workflow is not available.")
		return
	}
	...
	for _, response := range responses {
		rows = append(rows, []string{
			string(response.RunID), strconv.FormatUint(response.WorkflowVersion, 10), response.FormTitle,
			response.Field, response.Value, response.SubmittedAt.UTC().Format(time.RFC3339),
		})
	}
	writeCSV(w, "workflow-form-responses.csv",
		[]string{"run_id", "workflow_version", "form", "field", "value", "submitted_at"}, rows)
}

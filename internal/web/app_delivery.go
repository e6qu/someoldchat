...
// deliverySuccessRate formats delivered over the retained window. With nothing
// recorded it says so rather than reporting a misleading 0% or 100%.
func deliverySuccessRate(health domain.AppDeliveryHealth) string {
	total := health.DeliveredCount + health.FailedCount
	if total == 0 {
		return "—"
	}
	return strconv.Itoa(health.DeliveredCount*100/total) + "%"
}
...
.attempts{list-style:none;margin:14px 0 0;padding:0;display:grid;gap:8px}.attempts li{display:flex;gap:8px;align-items:center;font-size:14px;flex-wrap:wrap}.attempts time{margin-left:auto;color:var(--muted);font-size:12px}
...
<main class="layout">
  <div class="heading"><h1>Event delivery health</h1><p class="muted">Live durable queue state for <strong>{{.App.Name}}</strong>. This page reports acknowledgements, active work, scheduled retries, and a bounded history of the most recent delivery attempts with their outcomes.</p></div>
  <div class="stack">
    ...
    {{if .Health.Configured}}<section class="card" aria-labelledby="attempts-heading"><h2 id="attempts-heading">Recent delivery attempts</h2><dl class="facts"><dt>Delivered</dt><dd>{{.Health.DeliveredCount}}</dd><dt>Failed</dt><dd>{{.Health.FailedCount}}</dd><dt>Success rate</dt><dd>{{.SuccessRate}}</dd></dl>{{if .Health.RecentAttempts}}<ul class="attempts">{{range .Health.RecentAttempts}}<li><span class="dot {{if .Delivered}}ok{{else}}error{{end}}" aria-hidden="true"></span> Sequence {{.Sequence}}, attempt {{.Attempt}} — {{if .Delivered}}delivered{{else}}failed (<code>{{.Reason}}</code>){{end}} <time datetime="{{.AttemptedAt.Format "2006-01-02T15:04:05Z07:00"}}">{{.AttemptedAt.Format "Jan 2, 2006 15:04:05 UTC"}}</time></li>{{end}}</ul>{{else}}<p class="muted">No delivery attempts recorded yet. This history is bounded to the most recent outcomes and is not a complete ledger.</p>{{end}}</section>{{end}}
  </div>
</main>
{{end}}`
...
func (h Handler) developerAppDelivery(w http.ResponseWriter, r *http.Request) {
	...
	h.writeHTML(w, developerAppDeliveryTemplate, developerAppDeliveryData{
		App: app, Health: health, State: state, Summary: summary, SuccessRate: deliverySuccessRate(health),

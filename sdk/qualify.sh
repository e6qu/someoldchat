...
mkdir -p "$work/npm" "$work/python"
(cd "$root" && go build -o "$work/fixture" ./sdk/node-web-api/fixture)
...
cp "$root/sdk/node-web-api/qualification.mjs" "$work/node-web/qualification.mjs"
...
cp "$root/sdk/node-bolt/qualification.mjs" "$work/node-bolt/qualification.mjs"
...
PYTHONPATH="$work/python-slack-sdk" SAMEOLDCHAT_API_URL=http://127.0.0.1:18080/api/ python3 "$root/sdk/python-slack-sdk/qualification.py"
stop_fixture
start_fixture
...
PYTHONPATH="$work/python-bolt:$work/python-slack-sdk" SAMEOLDCHAT_API_URL=http://127.0.0.1:18080/api/ python3 "$root/sdk/python-bolt/qualification.py"
stop_fixture
start_fixture

mvn -q -f "$root/sdk/java-slack-api/pom.xml" dependency:go-offline compile
...
mvn -q -f "$root/sdk/java-slack-api/pom.xml" dependency:build-classpath -Dmdep.outputFile="$work/java-classpath"
SAMEOLDCHAT_API_URL=http://127.0.0.1:18080/api/ mvn -q -f "$root/sdk/java-slack-api/pom.xml" exec:java
SAMEOLDCHAT_API_URL=http://127.0.0.1:18080/api/ java -cp "$root/sdk/java-slack-api/target/classes:$(cat "$work/java-classpath")" sameoldchat.qualification.BoltQualification

...
| P1 | Administration browser coverage | Workspace administration is now registered unconditionally, so a deployment with no identity provider has it — the pages answer 403 to a member instead of 404 to everyone, and `authAdminRoleAllowed` reads the principal's own workspace rather than the login handler's, which also fixes administration being refused after an AUTH-02 workspace switch. ADMIN-01..03 still have no browser citation, for a second and independent reason: `cmd/server` grants the shared static session member scopes only, deliberately, because a shared bearer token must not carry control-plane authority. Exercising those journeys needs a fourth Playwright project with an administrator session the deployment will mint for a real identity, not for a static token. |
...
| P1 | Media transport | Huddles and calls have no WebRTC, audio, video or screen sharing of any kind. The huddle lifecycle is durable and first-class and every surface that offers a control says the media is absent rather than faking it. HUDDLE-02 and CALL-01 cannot be closed without building the transport. |

| P1 | Message-chrome accessibility | The message action toolbar is absolutely positioned at `top:-15px`, so it overlays the message above and axe reports its links (`Reply in thread`, `Copy link`) as `partiallyObscured` under WCAG 2.2 AA target-size (2.5.8). Target sizes and the permalink timestamp were raised to 24px in PR #139. Moving the toolbar into its own message's top padding was tried here and reverted: it destabilised the chrome enough that three unrelated journeys could no longer click a message action. The fix is a deliberate redesign of the hover-toolbar idiom with visual baselines, not a padding change. Discovered because **no accessibility scan had ever run on `/app` with a populated timeline** — every existing scan ran on list surfaces, so the product's most-used surface was unscanned. |
| P2 | Browser-citation ceiling | Four journeys cannot take a browser citation as the harness is built, and the reason is structural rather than unwritten tests. APP-04 and APP-06 are observable only at an app endpoint the browser harness does not host, and adding one to `cmd/server` would put test-only surface in the production binary. REMIND-04 needs `cmd/worker` beside a server sharing a durable store; the browser servers use `-store memory`, so a worker process would share nothing. REMIND-API-01 restricts itself to SDK evidence. Each is recorded in its own journey's Evidence section. The remaining browser gaps — ADMIN-01..03, ADMIN-05, AUTH-02, AUTH-05, CONNECT-02, CALL-01, HUDDLE-02, HUDDLE-04 — need an administrator session, an identity provider, or media, all of which are recorded separately above. |

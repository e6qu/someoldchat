...
| P2 | The channel header shows no member count | Slack's header carries the member count beside the topic, and clicking it opens the member list. This header shows the title, the topic and whether you have joined, with secondary actions behind one overflow control, but no count. Adding one is a data change rather than a markup one — the conversation view model carries no member count — so it is recorded rather than approximated from something already to hand. |

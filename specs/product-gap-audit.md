...
| P1 | `[ACT-02 ACT-03]` fails in CI about one run in four and has never been reproduced | One cause is fixed and proven: the journey identified its subject as `page.locator('.message').last()`, and a Playwright locator re-resolves on every use, so the reference moved to whichever message had just arrived. A probe reproduces that 12 times out of 12 positionally and 0 times out of 12 when the subject is named by its text. **Everything else previously recorded here was measured with an invalid instrument and is withdrawn.** The probe posted its churn into the conversation under test, which pushes the target message out of the newest rendered window; every failure it produced — at opening the picker, at the chip appearing, at the chip clearing — was that artefact. Directing the same churn at a different conversation gives 0 failures in 15, twice. Three client changes were measured against the noise and none is retained: the press-scoped refresh hold from PR #145, keyed reconciliation of the region's messages, and an authoritative mutation response (abandoned once the instrument was shown invalid). CI has failed at two different stages — the chip not clearing after a click, and the chip not appearing after one — and neither is explained. The journey's `toPass` retries convert it into a pass and are commented as a mitigation. CI now keeps Playwright's trace, DOM snapshot and network log for a failed browser run, which is what the next occurrence needs to be evidence rather than another guess. |

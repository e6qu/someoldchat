...
| P2 | Canvases and lists | List comments and files, list filtering, and a calendar layout remain. A list now has three layouts through a view switcher: the default single ordered list; a **board** that groups items into lanes by a select or checkbox column (a trailing lane holds items with no value, with a group-by chooser); and a **table** that heads the columns and sorts the rows by any of them — ascending then descending on the next click, each column kind sorted the way it reads (numbers numerically, a checkbox unchecked-first, dates chronologically), empties sinking to the bottom. Board and table render server-side over the whole list up to a bound (a larger list says so rather than look complete). Item actions from a board lane or table row return to the list layout for now. Canvases and lists are both searchable — a Lists tab matches a list's name and description prose under the same folded index, visibility rule, and query grammar as the Canvases tab. |

...
| P1 | Message-chrome accessibility | The message hover toolbar is absolutely positioned at `top:-15px`, so it overlays the message above it and axe reports its links (`Reply in thread`, `Copy link`) as `partiallyObscured` under WCAG 2.2 AA target-size (2.5.8). Target sizes and the permalink timestamp were raised to 24px in this change; the overlap is Slack's own overlapping-toolbar idiom and removing it is a layout decision, not a padding change. Discovered because **no accessibility scan had ever run on `/app` with a populated timeline** — every existing scan ran on `/app/dms`, `/app/activity` and other list surfaces, so the product's most-used surface was unscanned. The keyboard-shortcuts journey scans its own dialog and says so; an unscoped scan of a populated conversation is the follow-up. |

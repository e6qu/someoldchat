...
| P1 | A click inside the timeline can be lost under live-event churn | Two causes were tangled here and only one is fixed. **Fixed:** the journey identified its subject as `page.locator('.message').last()`, and a Playwright locator re-resolves on every use, so under this suite's shared session the reference moved to whichever message had just arrived and the assertions after a click were about a different message. A probe reproduces that 12 times out of 12 positionally and 0 times out of 12 when the subject is named by its text. **Open:** with the subject named, the same probe still fails about 3 rounds in 15 under forced churn, and on a failing round the network shows the reaction add and no remove — the press never reached the server. Two attempts changed nothing measurable: the press-scoped refresh hold added in PR #145 (3/15 without it, 4/15 with it) and keying the region's messages so unchanged nodes survive a refresh (which made the remove reach the server but left the rate unmoved). Both were reverted rather than kept on the hope they help. The journey retries its two mutations, which converts the failure into a pass; that is a mitigation and is commented as one. What has not been tried: making the mutation's own response the authoritative update for the region so no second fetch can replace it. |

...
| P2 | Canvases and lists | List comments and files, and list sorting and filtering, remain. A list now has more than one layout: alongside the default single ordered list, a **board** groups items into lanes by a select or checkbox column — a trailing lane holds items with no value — with a view switcher and a group-by chooser, server-rendered over the whole list up to a bound (a larger list says its lanes stop short rather than look complete). Item actions from a board lane return to the list layout for now. Canvases and lists are both searchable — a Lists tab matches a list's name and description prose under the same folded index, visibility rule, and query grammar as the Canvases tab. |

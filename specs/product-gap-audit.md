...
| P1 | `[ACT-02 ACT-03]` still fails occasionally, and is not reproducible locally | One cause here is fixed and proven: the journey identified its subject as `page.locator('.message').last()`, and a Playwright locator re-resolves on every use, so under this suite's shared session the reference moved to whichever message had just arrived and the assertions after a click were about a different message. A probe reproduces that 12 times out of 12 positionally and 0 times out of 12 when the subject is named by its text. **The rest of what PR #149 recorded here was wrong, and this corrects it.** That entry claimed a second, measured cause — "the network shows the reaction add and no remove, the press never reached the server" — at about 3 rounds in 15 under forced churn. Classifying the probe's failures by stage shows all of them land on opening the emoji picker (the target has scrolled out of reach after churn) or on the chip not rendering in time, and **none** on clicking the chip or the chip failing to clear, which are the stages the CI failure matches. The probe never reproduced the CI failure; the "no remove" observation was a round that never reached the remove. Three client changes were measured against that invalid instrument and all reverted: the press-scoped refresh hold from #145, keyed reconciliation of the region's messages, and making the mutation response authoritative (abandoned before implementation once the instrument was shown invalid). What is actually known: the journey fails in CI roughly one full run in four, only there, and its `toPass` retries convert it into a pass. A valid local reproduction is the prerequisite for any further attempt. |

...
| — | `[ACT-02 ACT-03]` intermittence (fixed, and the earlier diagnosis was wrong) | This journey went red about one full run in four and was recorded here as a product race: a control swapped out between resolving it and dispatching the click, with WebKit's not-focusing-a-clicked-button as the reason the existing guard missed it. That diagnosis was wrong. A probe that posts messages to force live refreshes while pinning reproduces it **12 times out of 12** when the subject is `page.locator('.message').last()`, and **0 times out of 12** when the same subject is named by its text under identical churn — with and without the press-scoped hold PR #145 added, which changes nothing measurable either way. A Playwright locator re-resolves on every use, and this suite's shared session keeps posting, so `.last()` silently moved to whichever message had just arrived and the assertions after a click were about a different message than the click. The journey now names its subject, the `toPass` retries that were masking it are gone so a regression fails loudly, and the hold #145 added on the wrong diagnosis is removed. |

...
| P2 | Browser-citation ceiling | Ten journeys have no browser citation, and each reason is structural rather than an unwritten test. ADMIN-01..03 and ADMIN-05 are now covered by a fourth Playwright project against a `-session-admin` server. Four cannot take a citation as the harness is built, and the reason is structural rather than unwritten tests. APP-04 and APP-06 are observable only at an app endpoint the browser harness does not host, and adding one to `cmd/server` would put test-only surface in the production binary. REMIND-04 needs `cmd/worker` beside a server sharing a durable store; the browser servers use `-store memory`, so a worker process would share nothing. REMIND-API-01 restricts itself to SDK evidence. Each is recorded in its own journey's Evidence section. The last five — AUTH-02, AUTH-05, CONNECT-02, HUDDLE-02, HUDDLE-04 — need an identity provider or media, both recorded separately above. CALL-01 was listed here after it had gained a citation; the count is now derived rather than described, so that cannot recur. The session-administration surface added later falls under the same identity-provider limitation: it lives on that page, so it is covered by web and cross-profile tests rather than a journey. |
| — | A staged file was announced as saved 450 ms before it was saved (fixed) | `[COMP-02 COMP-03 DRAFT-01 FILE-01 ACT-02]` failed on CI: after attaching a file and reloading, the draft came back with no attachment. Unlike the two journey races below, this one was the product. Staging a file uploaded it, then called `persistDraft()` — which only *schedules* the draft write on a 450 ms debounce — and announced “One file is saved with this draft” immediately. A member who saw that message and closed the tab within half a second lost the attachment, and the journey lost it by reloading. The debounce exists so typing does not post per keystroke; attaching a file is a discrete deliberate act, so it now writes at once and the announcement waits for that write to land. Removing a staged file announced the same way and is fixed with it. `saveDraftRemote` now returns its promise so a caller can sequence on it, which is what made the announcement honest rather than optimistic. |
| P2 | `[REMIND-01]` failed once on WebKit and has not repeated | Clicking "In 20 minutes" did not reach `/app/later?changed=reminder` within 30 s. One occurrence, on CI, on a branch that had just changed the composer's draft-persistence script — and the reminder path stages no files, so no mechanism connects them. It passed on the re-run of the same commit and passes locally on WebKit. Recorded rather than dismissed because a single unexplained navigation stall is exactly what the two navigation races above looked like before the third occurrence made them reproducible. The next occurrence should be read as evidence rather than noise. |
| — | Two journeys raced the navigation they had just started (fixed) | `[WORKFLOW-05]` failed twice on WebKit in CI with a 404 from the form-response CSV export. The cause was in the journey, not the product: it read `page.url()` immediately after clicking "Create workflow", so on the slowest engine the URL was still `/app/workflows` and the identifier taken from it was the literal `workflows` — the export then asked for a workflow of that name. Every other `page.url()` read in the suite waits for something first; this one did not, and now waits for the workflow URL exactly as `[WORKFLOW-02]` does after the same click. Recorded because the symptom was a 404 from an unrelated-looking export, which is the kind of failure that gets re-diagnosed as a product defect on its next appearance. `[WORKFLOW-02]` failed in the same CI run and then twice more, reporting a step type reverting to `function` after publish; it turned out to be the same class. It reloaded the page immediately after clicking Publish, with nothing in between, so on WebKit the reload aborted the in-flight POST and the reload showed the untouched draft — `function` is the type the workflow was created with. Every other publish in the suite is followed by an assertion that waits: three wait for the "Workflow published" notice, two fill a field that only exists after the navigation. This was the one site that followed a click straight with `page.reload()`, which does not wait for a pending navigation. Both fixes are waits the surrounding journeys already had, which is the tell that the suite's convention was right and these two sites had drifted from it. |
| — | The uncited-journey count was printed and never enforced (fixed) | `journeycheck` computed the journeys with no browser citation, printed `journeys.browser-gaps=…`, and returned nil. A journey losing its citation — somebody deleting a test — left the count quietly larger and the gate green, which is the same shape as the seam's parity backlog. It now holds to `browserGapCeiling`, failing when the set grows and when it shrinks without the ceiling being lowered; the ceiling is tested in both directions. Writing it down also caught the prose being stale: this table said six journeys needed an identity provider or media and named CALL-01 among them, which had gained a citation. |
| — | The seam parity backlog promised something nothing checked (fixed) | `parityGaps` carried the claim that "the set can only shrink" and a failure message telling the next person to "say why it cannot have one" — while the structure is a set of names with nowhere to say it, and 179 entries said nothing. Neither half was enforced: an entry could be added freely, and coverage won could drift back. The promise now has a mechanism, `parityGapCeiling`, which fails when the backlog grows *and* when it shrinks without the ceiling being lowered, so ground gained is kept. The ceiling was verified to fail in both directions rather than assumed to. Four backlog entries were closed with a real parity case in the same change, chosen by the priority the backlog itself records — methods that take a page bound — leaving 175. |
